...
import { type AIConfig, type AIProvider, getActiveAIKey, loadAIConfig, saveAIConfig } from '@/lib/aiConfig';
...
export default function SettingsPage() {
  ...
  // Required-setup attention indicators
  const aiKeyNeedsAttention = useMemo(() => {
    // Treat ollama as always-configured. For other providers, look at the
    // matching key on the in-memory aiConfig (mirrors getActiveAIKey logic but
    // reacts to live edits before save).
    const provider = aiConfig.provider;
    if (provider === 'ollama') return false;
    const keyMap: Record<AIProvider, string> = {
      openai: aiConfig.openaiKey,
      gemini: aiConfig.geminiKey,
      claude: aiConfig.claudeKey,
      ollama: '',
    };
    // Also fall back to persisted active key in case a save just happened.
    if (keyMap[provider]?.trim()) return false;
    const persisted = getActiveAIKey();
    if (persisted.provider === 'ollama') return false;
    return !persisted.apiKey?.trim();
  }, [aiConfig]);

  const companyProfileNeedsAttention = useMemo(
    ...
    () => [
      {
        ...
        description:
          'Required setup — set your AI provider key and company profile so AI features work.',
        items: [
          { id: 'ai-keys', label: 'AI API Keys', needsAttention: aiKeyNeedsAttention },
          { id: 'company-profile', label: 'Company Profile', needsAttention: companyProfileNeedsAttention },
        ],
      },
      {
        id: 'brand',
        label: 'Brand & voice',
        description: 'Brand & voice — tone and style guidance the AI applies to every draft.',
        items: [{ id: 'brand-voice', label: 'Brand Voice Editor' }],
      },
      {
        id: 'integrations',
        label: 'Integrations',
        description:
          'Integrations — connect publishing destinations and research sources used by the dashboard.',
        items: [
          { id: 'integrations', label: 'Integration Connectors' },
          { id: 'research-source', label: 'Research source' },
        ],
      },
      {
        id: 'advanced',
        ...
        description:
          'Advanced (preview) — early previews of compliance, audit, and security tools that are not yet wired up.',
        items: [
    ...
    <div className="space-y-6">
      <section className="surface-card p-6">
        ...
        <p className="mt-2 muted-copy">
          Configure the dashboard. Start with <span className="font-semibold text-slate-900">Required setup</span> —
          AI keys and your company profile must be filled before AI features will work.
        </p>
      </section>

      {/* Mobile / small-screen horizontal nav (scroll-snap pill row) */}
      <nav
        aria-label="Settings sections"
        className="surface-card lg:hidden -mx-1 overflow-x-auto p-3"
      >
          ...
          {settingsGroups.flatMap((group) =>
            group.items.map((item) => (
              <li key={item.id} className="snap-start shrink-0">
                <a
                  href={`#${item.id}`}
                  className="pill bg-white hover:bg-slate-50 inline-flex items-center gap-2"
                  style={{ borderColor: '#c7d6e5' }}
                >
      ...
      <div className="lg:grid lg:grid-cols-[16rem_minmax(0,1fr)] lg:gap-6">
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-24 space-y-5">
            {settingsGroups.map((group) => (
              <div key={group.id}>
                <p className="text-xs font-bold uppercase tracking-[0.2em] text-teal-700">{group.label}</p>
        ...
        {/* Main content column with grouped sections */}
        <div className="space-y-10 scroll-smooth">
          {/* GROUP 1 — Required setup */}
          <section aria-labelledby="group-required" className="space-y-4">
            <header>
              <h2 id="group-required" className="text-xs font-bold uppercase tracking-[0.2em] text-teal-700">
                Required setup
              </h2>
            ...
            {/* AI API Keys (now first under Required setup) */}
            <section id="ai-keys" className="surface-card p-6 scroll-mt-24">
              <div className="flex flex-wrap items-center gap-3">
                <h3 className="section-title">AI API Keys</h3>
                {aiKeyNeedsAttention ? (
                  <span className="pill bg-amber-50 text-amber-800" style={{ borderColor: '#f59e0b' }}>
                    Needs attention
                  </span>
                ) : null}
              </div>
              <p className="mt-2 muted-copy">
                Configure your AI provider and API keys for AI-powered features. Without a key for your active
                provider, draft generation and assistant tools will not work.
              </p>
            ...
            {/* Company Profile */}
            <section id="company-profile" className="surface-card p-6 scroll-mt-24">
              <div className="flex flex-wrap items-center gap-3">
                <h3 className="section-title">Company Profile</h3>
                {companyProfileNeedsAttention ? (
                  <span className="pill bg-amber-50 text-amber-800" style={{ borderColor: '#f59e0b' }}>
                    Needs attention
                  </span>
                ) : null}
              ...
              <div className="mt-4 rounded-2xl border border-teal-200 bg-teal-50/60 p-4">
                <h4 className="text-sm font-semibold uppercase tracking-[0.18em] text-teal-800">Auto-fill from website</h4>
          ...
          {/* GROUP 2 — Brand & voice */}
          <section aria-labelledby="group-brand" className="space-y-4">
            <header>
              <h2 id="group-brand" className="text-xs font-bold uppercase tracking-[0.2em] text-teal-700">
                Brand &amp; voice
              </h2>
              <p className="mt-1 muted-copy">
                Brand &amp; voice — tone and style guidance the AI applies to every draft.
              </p>
            </header>

            <section id="brand-voice" className="surface-card p-6 scroll-mt-24">
              <h3 className="section-title">Brand Voice Editor</h3>
              <p className="mt-2 muted-copy">
                A short, plainspoken description of your brand voice. Used as supplementary guidance for AI drafts.
              </p>
              <textarea
          ...
          <section aria-labelledby="group-integrations" className="space-y-4">
            <header>
              <h2 id="group-integrations" className="text-xs font-bold uppercase tracking-[0.2em] text-teal-700">
                Integrations
              </h2>
              <p className="mt-1 muted-copy">
                Integrations — connect publishing destinations and research sources used by the dashboard.
            ...
            <section id="integrations" className="surface-card p-6 scroll-mt-24">
              <h3 className="section-title">Integration Connectors</h3>
              <p className="mt-2 muted-copy">
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-2">
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <h4 className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-700">Per-user setup</h4>
                ...
                <div className="rounded-2xl border border-blue-200 bg-blue-50 p-4">
                  <h4 className="text-sm font-semibold uppercase tracking-[0.18em] text-blue-800">One-time app owner setup</h4>
                          ...
                          <div className="flex flex-wrap items-start justify-between gap-3">
                            <div>
                              <div className="flex items-center gap-2">
                                <h4 className="text-lg font-semibold text-slate-900">{connection.label}</h4>
            ...
            {/* Research source (Exa key) */}
            <section id="research-source" className="surface-card p-6 scroll-mt-24">
              <h3 className="section-title">Research source (Exa AI)</h3>
              <p className="mt-2 muted-copy">
          ...
          {/* GROUP 4 — Advanced (preview) */}
          <section aria-labelledby="group-advanced" className="space-y-4">
            <header>
              <h2 id="group-advanced" className="text-xs font-bold uppercase tracking-[0.2em] text-teal-700">
                Advanced (preview)
              </h2>
              <p className="mt-1 muted-copy">
                Advanced (preview) — early previews of compliance, audit, and security tools that are not yet wired up.
            ...
            <section
              id="compliance-flags"
              aria-disabled="true"
              className="surface-card p-6 scroll-mt-24 opacity-75"
            >
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="section-title">Compliance Flags</h3>
                <ComingSoonBadge />
              </div>
            ...
            >
              <div className="flex flex-wrap items-center gap-2">
                <h3 className="section-title">Audit Log Viewer</h3>
                <ComingSoonBadge />
              </div>
          ...
          {/* GROUP 5 — Session */}
          <section aria-labelledby="group-session" className="space-y-4">
            <header>
              <h2 id="group-session" className="text-xs font-bold uppercase tracking-[0.2em] text-teal-700">
                Session
              </h2>
              <p className="mt-1 muted-copy">Session — sign out of the dashboard.</p>
            </header>

            <section id="session" className="surface-card p-6 scroll-mt-24 flex flex-col items-start">
