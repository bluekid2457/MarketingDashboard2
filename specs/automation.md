...
- **Forward-looking — Bucket-A vs Bucket-B publishers.** The Math-Sans-Unicode converter (`frontend/src/lib/linkedinFormat.ts::markdownToLinkedInUnicode`, re-exported as `markdownToUnicodePost`; backend mirror `app.services.linkedin_text_format.markdown_to_linkedin_unicode`) is **platform-neutral**. When the Twitter and Instagram publishers ship, they MUST run their bodies through the same converter at publish time (Bucket A — plain-text feeds). Conversely, when the Medium, Newsletter, and Blog publishers ship, they MUST NOT apply the Unicode conversion — those destinations render markdown natively and a substituted body would publish double-formatted output. Today only LinkedIn is wired; the converter and the platform-neutral preview components (`UnicodePostPreview`, `MarkdownPostPreview`) are ready to plug in.
- **LinkedIn (connected user) behavior:**
  ...
  - Frontend converts the markdown body to LinkedIn-Unicode via `markdownToLinkedInUnicode(text)` BEFORE building the POST body. The converter is idempotent so the backend's defense-in-depth conversion (next step) is a safe no-op.
  - Calls `POST {NEXT_PUBLIC_API_URL}/api/v1/publish/linkedin/now` with `Authorization: Bearer <idToken>` and `{ userId, text: <converted>, visibility: 'PUBLIC' }`.
  - Backend verifies the ID token, **re-runs `markdown_to_linkedin_unicode` (defense-in-depth, idempotent)**, re-checks `len(converted) <= 3000` (raises HTTP 422 `payload_too_long` when strikethrough markdown pushes past the ceiling), delegates to `linkedin_publisher.publish_linkedin_text` (which decrypts the access token, reads `publishAuthorUrn`, posts to LinkedIn's `v2/ugcPosts` API), and returns `{ success, postUrn, postUrl }` (or a `{ success: false, error, status, providerError }` envelope per the backend error matrix).
...
`publish_one` is **framework-agnostic and never raises**. It performs the CAS to `publishing`, the `postUrn` idempotency short-circuit, the platform gate (`linkedin` only today), the markdown → LinkedIn-Unicode conversion (`linkedin_text_format.markdown_to_linkedin_unicode(text)` — applied to the resolved snapshot text BEFORE the platform-publisher call so the Firestore snapshot stays raw markdown), the call to `linkedin_publisher.publish_linkedin_text`, and the finalize update on the row (success or failure). The same function is invoked from the legacy `/publish/scheduled/run` route per-row loop so both paths share identical semantics. The idempotency short-circuit (existing `postUrn`) skips the converter entirely — there is nothing new to convert when the row was already published.
