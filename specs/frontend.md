...
frontend/
  ...
  src/
    app/
      ...
      page.tsx              # Marketing landing page â€” hero, three preview cards, feature grid, workflow steps, CTA
      page.tsx              # Redirect-only — server redirects / to /landingPages/index.html (see next.config.js)
      api/
        ideas/
          rationale/
      ...
      (auth)/
        login/
          page.tsx          # Screen 1 â€” Login & Authentication
        register/
      ...
      (app)/
        ...
        dashboard/
          page.tsx          # Screen 2 â€” Dashboard overview
        ideas/
          page.tsx          # Screen 3 â€” Idea Input & Backlog
        angles/
          page.tsx          # Screen 4 â€” AI Angle Selection & Outline
        storyboard/
          page.tsx          # Screen 5 â€” Storyboard Index (replaces legacy /drafts list)
          new/
            page.tsx        # Redirect â†’ /storyboard
          [id]/
            page.tsx        # Screen 5b â€” Storyboard Editor (dynamic [id])
        drafts/
          page.tsx          # Redirect â†’ /storyboard (legacy)
          new/
            page.tsx        # Redirect â†’ /storyboard (legacy)
          [id]/
        ...
        analytics/
          page.tsx          # Screen 9 â€” Analytics & Performance
        collaboration/
          page.tsx          # Screen 10 â€” Collaboration & Client Management
        settings/
          page.tsx          # Screen 11 â€” Settings & Compliance
        notifications/
          page.tsx          # Screen 12 â€” Error & Notifications
    components/
...
### Root marketing page
- `src/app/page.tsx` — redirect-only server component. Calls `next/navigation` `redirect()` pointing to `/landingPages/index.html`. The actual landing page content lives at `frontend/public/landingPages/index.html` (pure HTML, no React/JSX).
- next.config.js enforces an `async redirects()` rule (source: `/`, destination: `/landingPages/index.html`, permanent: false) so the redirect fires even before the server component runs.
- Access control for authenticated routes is enforced by the `(app)` layout guard.
...
- **`(app)`** â€” authenticated routes wrapped by `AppLayout` which renders `<Nav />` + `<main>` only after Firebase auth verification.
...
- Sidebar groups (in order):
  ...
  2. **Pipeline** â€” the canonical six pipeline steps in canonical order: **Ideas â†’ Angles â†’ Storyboard â†’ Adapt â†’ Review â†’ Publish**. The labels and order are imported from `src/lib/pipeline.ts` (`PIPELINE_STEPS`) so the sidebar, the WorkflowStepper, and any per-page breadcrumbs always agree. Never re-declare these labels inline.
  3. **More** â€” `Analytics`, `Collaboration`, `Settings`, `Notifications`. The `/collaboration` link is labeled `Collaboration` (NOT `Audience`); `Campaigns`/`Content`/`Audience` are no longer used as sidebar labels anywhere.
...
- Mobile pill row renders the full flattened list (Dashboard + Pipeline + More) using the same labels â€” no separate mobile-only naming.

### `src/app/(app)/layout.tsx` â€” AppLayout
...
- Displays the 6 labelled steps from `PIPELINE_STEPS` (see `src/lib/pipeline.ts`): **Ideas â†’ Angles â†’ Storyboard â†’ Adapt â†’ Review â†’ Publish**. Step labels and ordering are imported from that module â€” the stepper never re-declares the array inline.
...
- When no matching same-idea angle context exists, Storyboard and Adapt links fall back to `/angles?ideaId={ideaId}` (when `ideaId` is known) or `/angles` (when no idea context exists) â€” never to bare `/storyboard/{id}` or `/adapt/{id}` routes without `angleId`, which would cause those pages to fail.
- Responsive: step labels hidden on mobile (xs screens), shown from `sm` breakpoint up.

### `src/lib/pipeline.ts` â€” Canonical Pipeline Source-of-Truth
- Exports `PIPELINE_STEPS`, a typed `readonly` array of `{ key, label, path, paths }` describing the canonical content pipeline in this exact order: `ideas â†’ angles â†’ storyboard â†’ adapt â†’ review â†’ publish`.
- Exports the `PipelineStepKey` union and `PipelineStep` shape for typed consumers.
- Single import source for `Nav.tsx` (sidebar Pipeline group) and `WorkflowStepper.tsx` (sticky stepper). Per-page breadcrumbs (Storyboard, Adapt) must use the same labels, with `Publish` as the final word â€” never `Schedule`.
- Adding/removing/renaming a pipeline step is a one-line edit here; nav, stepper, and breadcrumbs all update together.

### `src/lib/workflowContext.ts` â€” Workflow Context Persistence
...
- Used directly on the dashboard KPI tiles ("Engagement rate", "Best post type") and on every `<h3>` inside the Settings â†’ Advanced (preview) group (Compliance Flags, Audit Log Viewer, Security Settings).
...
### Screen 1 â€” Login & Authentication (`/login`)
...
### Screen 2 â€” Dashboard (`/dashboard`)
...
**Sections:**
0. Get started checklist (first-run onboarding card, rendered as the FIRST card above the hero) â€” title "Get started with Flowrite", subtitle "A few quick steps before AI features will work.". Three rows, each with a âœ… green check OR â¬œ empty box on the left, the step text in the middle, and a primary "Open" link (or outlined "Review" link when done) on the right. Items and "done" criteria:
   1. "Set your AI provider key" â†’ done when `getActiveAIKey()` returns a non-empty `apiKey`, OR when active provider is `ollama` (no key required). Action link: `/settings#ai-api-keys`.
   2. "Fill in your Company Profile" â†’ done when `loadCompanyProfile(currentUid)` from `frontend/src/lib/companyProfile.ts` returns a profile with non-empty `companyName`. Action link: `/settings#company-profile`.
   3. "Add your first idea" â†’ done when the user has at least one document in `users/{uid}/ideas`. Probed via a one-time Firestore `getDocs(query(collection(db, 'users', uid, 'ideas'), limit(1)))` (existence check only â€” the full list is not loaded). Action link: `/ideas`.
   - Auto-hide: when all three items are `done`, the entire checklist card renders `null` (no manual dismiss button â€” completing the steps IS the dismissal).
...
### Screen 3 â€” Idea Backlog (`/ideas`)
**Route:** `src/app/(app)/ideas/page.tsx`
**Layout Notes:** Full-width single-column layout â€” no sidebar. Top-to-bottom: header card â†’ WorkflowStepper â†’ input card â†’ filter tabs + sort â†’ idea cards.
**Sections:**
1. Header card: breadcrumb (`CAMPAIGNS Â· IDEAS`), page title (`Idea Backlog`), subtitle, and stats row showing total backlog count, strong-rated count, and last-scored time label (derived per render from `relevance.scoredAtMs`).
2. `<WorkflowStepper />` unchanged.
3. Input card: single `<input type="text">` (one-sentence topic), three pill-style inline `<select>` dropdowns (Tone, Audience, Format â€” options `['Any','Article','Post','Thread','Newsletter','Video Script']`, default `'Any'`), "Score only" outline button (runs `submitIdea(true)` â€” skips `generateIdeaRationale`, uses `scoreIdeaTopic` + `buildFallbackRationale` only), and "Add & score" primary button (existing full AI flow). `format` state value is saved alongside `tone`/`audience` in the Firestore `addDoc` payload.
4. Filter tabs: pill buttons for `All`, `Strong`, `Moderate`, `Weak`, `No angles yet` â€” each showing a count. `No angles yet` = ideas where `!draftMap.has(idea.id) && !adaptMap.has(idea.id)`. `draftMap` (`Map<string, string>`, ideaId â†’ angleId) is populated from `users/{uid}/drafts`; `adaptMap` (`Map<string, string>`, ideaId â†’ angleId) from `users/{uid}/adaptations` â€” both fetched on `currentUser` change via `getDocs` with `Promise.all`; empty Maps used on error.
5. Sort dropdown on filter row (right): `Score high â†’ low` (default), `Newest`, `Oldest`, `Topic A-Z`. `ratingFilter` state replaces the old `toneFilter`; `visibleIdeas` memo filters by rating label or no-draft/adapt status.
6. Idea cards (replaces table): each idea is a rounded-2xl white card with a 56Ã—56 px color-coded score circle (emerald=Strong, amber=Moderate, rose=Weak, slate=unscored), tone/audience pills + date + live signals count in the tag row, bold title, secondary topic line when title differs, and a right-column action group consisting of exactly ONE primary CTA button (dark-green filled `#1a7a5e`) plus a "..." overflow dropdown. The primary CTA is selected via the idea's furthest pipeline state: (a) `adaptMap.has(idea.id)` â†’ "Resume in Adapt â†’" routing to `/adapt/{ideaId}?angleId={angleId}`; (b) `draftMap.has(idea.id) && !adaptMap.has(idea.id)` â†’ "Resume in Storyboard â†’" routing to `/storyboard/{ideaId}?angleId={angleId}`; (c) neither map has the idea â†’ "Open angles â†’" routing through `openAnglesForIdea`. `setWorkflowContext({ ideaId, ideaTopic })` is called before navigation. The "..." overflow dropdown always contains "Edit title" and "Delete"; when the idea has an adaptation it also contains "Open angles" and (if a draft also exists) "Open Storyboard"; when only a draft exists it also contains "Go to Adapt". The "..." trigger button uses `aria-haspopup="menu"` and `aria-expanded={isMenuOpen}`; the menu container uses `role="menu"` and each item button uses `role="menuitem"`. `openMenuIdeaId` state continues to gate single-open behavior. Inline title-edit (Save/Cancel) replaces the entire action group while active.
...
### Screen 4 â€” AI Angle Selection & Outline (`/angles`)
...
### Screen 5a â€” Storyboard Index (`/storyboard`)
...
2. Recycling banner: derives a per-record "fresh / refresh / repurpose" verdict from `updatedAtMs` (â‰¥ 60 days â†’ refresh; â‰¥ 120 days â†’ repurpose) and surfaces a yellow banner listing the top 6 stale records with one-click `Refresh` / `Repurpose` deep links into `/storyboard/{ideaId}?angleId={angleId}`.
...
- `/drafts` (`src/app/(app)/drafts/page.tsx`) â€” server component that calls `redirect('/storyboard')` (legacy URL).
- `/adapt/new` (`src/app/(app)/adapt/new/page.tsx`) â€” **Adapt landing page** (client component). No longer a silent redirect. Reads `uid` from `onAuthStateChanged`, then streams two collections in parallel via Firestore `onSnapshot`:
  - `users/{uid}/adaptations` ordered by `updatedAt` desc â€” used to render the "Resume an existing adaptation" section. Each card shows `ideaTopic`, `angleTitle`, a chip row of platforms that have non-empty saved copy (LinkedIn / X / Twitter / Medium / Newsletter / Blog), and a primary "Resume â†’" button linking to `/adapt/{ideaId}?angleId={angleId}`. Empty state renders the copy "No adaptations yet."
  - `users/{uid}/drafts` ordered by `updatedAt` desc â€” used to render the "Start a new adaptation from a storyboard" section. Storyboards whose `{ideaId}_{angleId}` key already appears in the adaptations collection are filtered out so the user only sees storyboards that don't already have an adaptation. Each card shows `ideaTopic`, `angleTitle`, and a primary "Adapt this â†’" button linking to `/adapt/{ideaId}?angleId={angleId}` (the dynamic Adapt page handles the "no adaptation yet" case via its existing fallback path). Empty state renders "Finish a storyboard first to adapt it."
  - When the user has zero storyboards at all, a third small hint card appears with a link back to `/storyboard` ("Go to Storyboard â†’").
...
- `/storyboard/new` (`src/app/(app)/storyboard/new/page.tsx`) â€” **Storyboard-new landing page** (client component). Reads `uid` from `onAuthStateChanged`, then probes `users/{uid}/drafts` with `limit(1)`. If the user already has at least one storyboard, calls `router.replace('/storyboard')` so the existing index lists their work; if they have zero, renders an explainer card with the copy "A storyboard is the long-form draft we generate from your selected angle." plus a primary CTA "Pick an idea â†’" linking to `/ideas`. Loading state renders `<Spinner size="sm" />`.
- `/drafts/new` (`src/app/(app)/drafts/new/page.tsx`) â€” same shape as `/storyboard/new` (zero-storyboard explainer card + redirect to `/storyboard` when at least one exists). Kept so legacy deep links land on a guided page instead of a silent redirect.
...
### Screen 5 â€” Draft Editor (`/drafts/[id]`)
...
4. Auto-saves to Firestore 2 s after each keystroke (debounced); save status shown in header ("Savingâ€¦" / "âœ“ Saved at HH:MM")
...
7. Word count pill in the editor toolbar; "â†º Regenerate" and "ðŸ¤– AI Chat" toggle buttons
...
9. Action buttons: Save Draft, **ðŸŽ¯ Adapt for Platforms**, Submit for Review, Schedule Post. The Adapt action is disabled until a draft exists, writes `localStorage['adapt_draft_context'] = { ideaId, angleId, idea, selectedAngle, draftContent }`, and routes to `/adapt/<ideaId>?angleId=<angleId>`.
10. **Content Analysis section** â€” three analysis buttons (each calls `POST /api/drafts/analyze`):
   - **ðŸ“ˆ SEO Optimizer** (`type=seo`): primaryKeyword, secondaryKeywords, keywordDensity, readabilityScore, readabilityGrade, metaDescription, titleSuggestions, optimizationTips, similarArticleTopics (SEO coverage gaps), wordCount
   - **ðŸ” Plagiarism / AI Check** (`type=plagiarism`): aiLikelihoodScore (0â€“100), aiLikelihoodLabel, flaggedPhrases[], humanizationTips[], originality, verdict
   - **ðŸ”— Source Check** (`type=sources`): claims[] (claim + needsCitation + suggestedSearchQuery), relevanceScore, relevanceSummary, urlsFound[], recommendations[]
...
### Screen 5b â€” Storyboard Editor (`/storyboard/[id]`)
...
**Layout Notes:** Single editor workspace with one source-of-truth textarea, a floating in-place inline AI editor, an in-editor sentence-diff stack for AI chat proposals, and a right-side AI change timeline for rollback. A sticky `<DocumentContextHeader />` ("Editing: <idea topic> Â· <angle title>" with a right-aligned "Step 3 of 6" pill) renders directly beneath the WorkflowStepper so the user keeps a persistent anchor across the Storyboard â†’ Adapt â†’ Publish jump.
**Breadcrumb:** `Angles â†’ Storyboard (Active) â†’ Adapt â†’ Review â†’ Publish` â€” labels and final word match the canonical `PIPELINE_STEPS` (never `Schedule`).
**Sections:**
...
### Screen 6 â€” Multi-Channel Adaptation (`/adapt/[id]`)
...
**Layout Notes:** Platform generation + tabbed editor workspace; each platform editor supports floating inline edit controls, an embedded AI chat panel that emits sentence-level in-editor diffs, and a right-side AI timeline that restores prior AI-applied states for the active platform. A sticky `<DocumentContextHeader />` ("Editing: <idea topic> Â· <angle title>" with a right-aligned "Step 4 of 6" pill) renders directly beneath the WorkflowStepper so the user keeps the same persistent editing anchor used on Storyboard.
**Breadcrumb:** `Angles â†’ Storyboard â†’ Adapt (Active) â†’ Review â†’ Publish` â€” labels and final word match the canonical `PIPELINE_STEPS` (never `Schedule`).
**Sections:**
...
15. Optimization tools call `POST /api/drafts/analyze` against the active platform text and render visible result panels for:
   - `ðŸ“ˆ SEO Optimizer`
   - `ðŸ” AI Check` (backed by `type='plagiarism'`)
   - `ðŸ”— Source Check`
...
### Screen 7 â€” Publishing & Scheduling (`/publish`)
**Route:** `src/app/(app)/publish/page.tsx`
**Layout Notes:** Library-style scheduling workspace that lists every saved adaptation for the signed-in user, with per-adaptation platform cards, edit/delete controls, schedule pickers, and a global upcoming schedule list. When the user arrives via Adapt's "Save and continue" CTA (i.e. `workflow_context` localStorage holds a single `ideaId` + optional `angleId`), a sticky `<DocumentContextHeader />` ("Editing: <idea topic> Â· <angle title>" with a right-aligned "Step 6 of 6" pill) renders at the top of the page so the editing anchor persists from Storyboard â†’ Adapt â†’ Publish. The header is hidden in the multi-adaptation library view (no workflow context).
**Sections:**
...
14. Each upcoming scheduled post in the `Upcoming Scheduled Posts` list has a per-row `Cancel` button. Click reveals an inline two-step confirm — red `Confirm cancel` + neutral `Keep` buttons. Confirming calls `cancelScheduledPost(id)` from `frontend/src/lib/publish.ts` which `DELETE`s `/api/v1/publish/schedule/{id}` with the user's Firebase ID token. On success the Firestore listener removes the row from the list automatically; a success toast renders. On 409 `status_not_cancellable` the user is told the post has already left the queue. On 404 `not_found` the user is told the post may have already been cancelled. The same Cancel/Remove control is rendered on rows in the `Failed Scheduled Posts` list (button label is `Remove` / `Confirm remove` / `Keep`) and uses the same wrapper.
...
### Screen 8 â€” Review & Approval Workflow (`/review`)
...
### Screen 9 â€” Analytics & Performance (`/analytics`)
...
### Screen 10 â€” Collaboration & Client Management (`/collaboration`)
...
### Screen 11 â€” Settings & Compliance (`/settings`)
**Route:** `src/app/(app)/settings/page.tsx`
**Layout Notes:** Sticky in-page nav (left rail at `lg:` and up; horizontal scroll-snap pill row on mobile) groups settings into five ordered groups with anchor-link smooth scrolling. Each section keeps its own `surface-card` container and uses an `id` matching the nav anchor. All sections remain reachable by scrolling â€” the nav does NOT hide content behind a click. Required-setup nav items render an amber "needs attention" dot (and the section header shows a "Needs attention" pill) when the setting is unconfigured: AI API Keys lights up when the active provider has no key (ollama is treated as always-configured, otherwise checks `getActiveAIKey()` from `src/lib/aiConfig.ts` plus the live in-memory key), and Company Profile lights up when `companyName` is empty (using the cached `loadCompanyProfile`).
**Section groups (ordered):**
1. **Required setup** â€” sections every user must complete for the app to work.
   1. **AI API Keys** (`#ai-keys`) â€” select active provider (OpenAI / Gemini / Claude / Local Ollama); provider key inputs plus Ollama base URL and model; Save button calls `saveAIConfig` and shows "Saved!" toast; config loaded via `useEffect` from `loadAIConfig()` in `src/lib/aiConfig.ts`. First card under Required setup so a new user does not have to scroll past placeholder cards.
   2. **Company Profile** (`#company-profile`) â€” full Company Profile form (companyName, industry, description, products, services, valueProposition, targetMarket, keyDifferentiators, brandVoice). Includes the "Auto-fill from website" panel that POSTs to `/api/company/autofill` using the active AI provider key. Save persists via `saveCompanyProfile` to Firestore at `users/{uid}.companyContext` and the local cache.
2. **Brand & voice**
   1. **Brand Voice Editor** (`#brand-voice`) â€” textarea persisted alongside the brand-voice field. (Currently a non-persisted textarea; the canonical brand voice is the `brandVoice` field on `users/{uid}.companyContext` written by Company Profile.)
3. **Integrations**
   1. **Integration Connectors** (`#integrations`) â€” LinkedIn connect/disconnect flow plus per-user and app-owner setup guidance (full behavior documented below).
   2. **Research source** (`#research-source`) â€” optional Exa AI key field; saved via `saveExaKey` (`src/lib/exaConfig.ts`) to localStorage; when present, drafts use Exa instead of DuckDuckGo for source retrieval.
4. **Advanced (preview)** â€” placeholder/preview features grouped together so they can be labelled "Coming soon" in a follow-up UX pass.
   1. **Compliance Flags** (`#compliance-flags`) â€” placeholder.
   2. **Audit Log Viewer** (`#audit-log`) â€” placeholder.
...
- `saveAIConfig(config)` â€” persists to `localStorage['ai_config']`
- `loadAIConfig()` â€” reads from localStorage, returns defaults on missing/error
- `getActiveAIKey()` â€” returns `{ provider, apiKey }` for the current active provider
...
- API:
  - `loadCompanyProfile(userId | null)` â€” loads from Firestore when uid provided, else returns the local cache. Falls back to cache if Firestore is unavailable or empty.
  - `saveCompanyProfile(uid, profile)` â€” writes Firestore doc (merge) + cache.
  - `loadCompanyProfileFromCache()` â€” synchronous cache-only read.
  - `companyProfileToContextLines(profile)` â€” returns `string[]` of "Field: value" lines for non-empty fields; suitable to send as `companyContext: string[]` to AI endpoints.
  - `companyProfileToTrendTerms(profile, limit?)` â€” returns short keyword/phrase tokens (length 2â€“60) extracted from `industry`, `products`, `services`, `targetMarket`, and `keyDifferentiators`, deduplicated and capped (default `limit = 6`). Used by the Ideas and Angles pages to bias the `/api/trends` search via the `companyTerms` query string.
  - `companyProfileToPromptBlock(profile)` â€” returns a single multi-line "Company context:" block, or `null` when empty.
...
### Screen 12 â€” Error & Notifications (`/notifications`)
...
- `/api/drafts/research` accepts `{ provider, apiKey, ollamaBaseUrl?, ollamaModel?, topic?, audience?, draft?, query?, companyContext? }`. The user prompt appends a "Company context" block so the brief biases toward findings that fit the company's industry, audience, and product. The DuckDuckGo web-search query itself is NOT modified by company context â€” only the synthesis prompt is.
