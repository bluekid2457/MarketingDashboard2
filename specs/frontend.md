...
| Multi-Channel Adaptation | Platform Selector | TODO |
| Multi-Channel Adaptation | Preview Per Format | TODO |
| Multi-Channel Adaptation | AI Chat for Editing | TODO |
...
| Review & Approval Workflow | Draft Queue | TODO |
...
- While auth state is resolving, shows a loading state (`Checking your session...`).
...
3. Submit loading state with duplicate-submit prevention
...
3. Firebase Email/Password `createUserWithEmailAndPassword`
4. Auth-state redirect (signed-in users route to `/dashboard`)
5. Inline user-safe error banner
...
3. Firestore-backed ideas table scoped to the signed-in user
4. Functional sort and filter controls (sort by newest/oldest/topic, filter by tone and format)
5. Selected-idea summary card derived from persisted Firestore data
6. "Generate Angles" CTA in Selected Idea card routes to `/angles?ideaId=<ideaDocId>`
7. Live trend snapshot and right-hand articles panel sourced from `/api/trends`
...
**Sections:**
1. Resolves `ideaId` from query string, then loads `users/{uid}/ideas/{ideaId}` from Firestore for signed-in user
...
5. Manual "Save Draft" button triggers an immediate Firestore write; Firestore doc schema: `{ content, ideaId, angleId, ideaTopic, angleTitle, status: 'draft', createdAt, updatedAt }`
6. Word count pill in the editor toolbar; "↺ Regenerate" and "🤖 AI Chat" toggle buttons
7. **AI Chat Panel** (toggle, `POST /api/drafts/chat`): conversational assistant with full draft context; AI returns edits wrapped in `<UPDATED_DRAFT>` tags; banner offers "Apply Changes" / "Discard"; chat history in component state
8. Action buttons: Save Draft, Submit for Review, Schedule Post
9. **Content Analysis section** — three analysis buttons (each calls `POST /api/drafts/analyze`):
...
**Layout Notes:** Platform chips at top, preview pane and AI chat in split layout.
**Sections:**
1. Platform Selector (LinkedIn, Twitter/X, Medium, Newsletter, Instagram pill buttons) (TODO)
2. Preview Per Format (TODO)
3. AI Chat for Editing (TODO)
...
7. Sign out action (Firebase `signOut`) from the Session section
...
- `/api/drafts` accepts `{ provider, apiKey, ollamaBaseUrl?, ollamaModel?, idea, angle }`, builds a long-form draft prompt from selected idea + angle outline, calls the configured AI provider, and returns `{ draft, provider }`.
- `/api/drafts` accepts `{ provider, apiKey, ollamaBaseUrl?, ollamaModel?, idea, angle }`, builds a long-form draft prompt from selected idea + angle outline, calls the configured AI provider, and returns `{ draft, provider }`.
...
- `/api/drafts/analyze` accepts `{ provider, apiKey, ollamaBaseUrl?, ollamaModel?, draft, type: 'seo' | 'plagiarism' | 'sources' }`, runs type-specific analysis prompts, logs the prompt and raw provider response to the server console, robustly parses the AI's JSON response (code-fence fallback included), and returns `{ type, result, provider }`.
